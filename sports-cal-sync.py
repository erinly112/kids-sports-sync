...
def send_deletion_email(deleted):
    """Email Erin via Rosie listing events auto-deleted from the target calendar."""
    try:
        lines = [
        ...
        rosie_mailer.send_plain(
            subject=f"Sports sync: {len(deleted)} event(s) auto-deleted from Kid Activities",
            body="\n".join(lines),
            to=NOTIFY_EMAIL,
        )
